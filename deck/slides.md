...
- Across the five Flash Reports we parsed (2025-12 to 2026-07) we found 965 contradictions in the Ministry's own published numbers — spread over those five reports, not all inside one of them — e.g. project 705410: cumulative expenditure ₹89,486.62 crore in one report, ₹2,384.29 crore in the next (185 such projects).
...
- Prediction layer (open-source scikit-learn): slip-filing classifier tested on the report it never saw (recall@100 0.28 vs 0.16 for logistic regression), next-month progress forecaster (MAE 1.5 vs 2.3 for linear extrapolation), overrun drivers and peer benchmark (OLS vs gradient boosting), and 134 statistical outliers from an isolation forest — model output, shown beside the rule-based ledger and never counted into its total.
...
- Held-out July: n = 1732, projects that filed a revision = 334; on the report-only feature set A, gradient boosting scores PR-AUC 0.61 against 0.38 for logistic regression — the gain is the model family, not extra features.
...
- ₹340,503.33 crore of recorded cost overrun in the 2026-07 panel; 965 rule-based arithmetic contradictions across the five reports parsed, which nobody currently flags; 443 projects left the panel across the period without a public reason.
