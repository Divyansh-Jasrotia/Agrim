...
- In the Ministry's own 2026-07 report we found 1101 arithmetic impossibilities across 1775 projects — e.g. project 705410: cumulative expenditure ₹89,486.62 crore in one report, ₹2,384.29 crore in the next (185 such projects).
- Audit layer (no model): contradiction ledger, exit ledger (443 projects left the panel), unreachable-date early warning (1289 projects), field-information audit, transparent risk score.
- Prediction layer (open-source scikit-learn): slip-filing classifier tested on the report it never saw (recall@100 0.28 vs 0.15 for logistic regression), next-month progress forecaster (MAE 1.5 vs 2.3 for linear extrapolation), overrun drivers and peer benchmark (OLS vs gradient boosting).
...
- Held-out July: n = 1732, projects that filed a revision = 334; PR-AUC 0.59 (boosting + audit features) vs 0.35 (logistic).
...
- ₹340,503.33 crore of recorded cost overrun in the 2026-07 panel; 1101 inconsistencies nobody currently flags; 443 projects left the panel across the period without a public reason.
