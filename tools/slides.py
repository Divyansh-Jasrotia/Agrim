...
- Across the five Flash Reports we parsed ({first_snapshot} to {latest_snapshot}) we found {contradictions_arithmetic} contradictions in the Ministry's own published numbers — spread over those five reports, not all inside one of them — e.g. project {opener_code}: cumulative expenditure ₹{opener_before_cr} crore in one report, ₹{opener_after_cr} crore in the next ({count_EXP_DECREASE} such projects).
