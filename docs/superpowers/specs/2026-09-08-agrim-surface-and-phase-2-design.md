...
**The word "impossibilities" also overreaches, and Task 21 resolves it by dropping the word.** Only `EXP_DECREASE` (185) and `PROG_DECREASE` (158) are true impossibilities — a cumulative field cannot fall. `EXP_GT_REVISED_COST` (331) can occur legitimately when an agency overspends against an unrevised sanction; `ZERO_PROG_NONZERO_EXP` (285) is normal during land acquisition and mobilisation; `DOC_BEFORE_APPROVAL` (8) is a data-entry contradiction rather than an arithmetic one.

**Decision (Ranvir, 9 Sept): the word is "contradictions" everywhere — never "impossibilities".** It is accurate for all five types, survives a hostile question, and needs no sub-split. The headline is **967 contradictions** and, separately, **134 statistical anomalies (model)**. Applies to every screen, `deck/numbers.json`, the deck and the spoken pitch. Individual flag sentences may still say a cumulative field cannot fall, because for those two types it is true.
